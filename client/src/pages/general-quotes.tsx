...
export default function GeneralQuotes() {
  ...
  useEffect(() => {
    if (!authLoading && (!isAuthenticated || ((user as any)?.role !== 'admin' && (user as any)?.role !== 'employee'))) {
      toast({
        ...
        description: "Dostęp do wycen wynajmu ogólnego jest dostępny tylko dla pracowników i administratorów.",
        variant: "destructive",
      });
      setTimeout(() => {
  ...
  const { data: quotes = [], isLoading } = useQuery<GeneralQuote[]>({
    queryKey: ["/api/general-quotes"],
    enabled: isAuthenticated && (user as any)?.role === 'admin',
  });

  const deleteQuoteMutation = useMutation({
    mutationFn: async (quoteId: number) => {
      const response = await apiRequest(`/api/general-quotes/${quoteId}`, "DELETE");
      return response;
    },
    onSuccess: () => {
    ...
    <div className="min-h-screen p-4">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground flex items-center gap-2">
              ...
              Wyceny Wynajmu Ogólnego
            </h1>
            <p className="text-muted-foreground mt-2">
              Zarządzaj wycenami sprzętu wynajmu ogólnego
            </p>
          </div>
          <Link href="/create-general-quote">
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Wyceny wynajmu ogólnego ({filteredQuotes.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {filteredQuotes.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-muted-foreground mb-2">
                  {searchTerm || statusFilter !== 'all' ? 'Brak wyników' : 'Brak wycen wynajmu ogólnego'}
                </h3>
                <p className="text-muted-foreground mb-4">
                  {searchTerm || statusFilter !== 'all' 
                    ? 'Spróbuj zmienić kryteria wyszukiwania'
                    : 'Utwórz pierwszą wycenę sprzętu wynajmu ogólnego'
                  }
                </p>
                {!searchTerm && statusFilter === 'all' && (
                  <Link href="/create-general-quote">
                  ...
                  <TableBody>
                    {filteredQuotes.map((quote) => (
                      <TableRow key={quote.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Link href={`/general-quotes/${quote.id}`}>
                              <Button variant="outline" size="sm" title="Podgląd">
                                <Eye className="w-4 h-4" />
                              </Button>
                            </Link>
                            <Link href={`/general-quotes/${quote.id}/edit`}>
                              <Button variant="outline" size="sm" title="Edytuj">
                            ...
                            <Button
                              ...
                              size="sm"
                              title="Drukuj"
                              onClick={() => window.open(`/api/general-quotes/${quote.id}/print`, '_blank')}
                            >
